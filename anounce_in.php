...
<body>
	...
	<div class="mt-5" style="background-color: rgba(255, 255, 255, 0.973);">
		<div class="container col-sm-12 pt-5">
			<h1 class="text-center font-italic font-weight-bold pb-3 small">Annoncement</h1>
			<form action="" method="POST">
				<label for="" class="font-weight-bold">Type samething</label>
				<textarea id="" cols="30" rows="10" name="text1" placeholder="Fill the text here.........." class="form-control border-warning" style="resize:none;"></textarea>
				<button type="submit" name="save" class="btn btn-outline-warning w-25 mt-4">Send</button>
			</form>
			<?php
			include "connection.php";
			if(isset($_POST['save'])){
				$announce = $_POST['text1'];
				if(empty($announce)){
					echo "<p class='text-center text-danger small'>Empty field</p>";
				}else{
					$insert = mysqli_query($con,"INSERT INTO announce_user VALUES('','$announce',NOW())");
				}
			}
			?>
			<table class="table table-bordered table-hover my-5 table-responsive">
				<tr>
					<th>No</th>
					<th>User Output</th>
					<th>Date&Time</th>
			...
			<?php
				...
				while($rows = mysqli_fetch_array($select)){
					...
					<tr>
						<td><?php echo $x?></td>
						<td><?php echo $rows['text_user']?></td>
						<td><?php echo $rows['date_time']?></td>
						<td><a href="delete.php?link=<?php echo $rows['announceid']?>" class='text-warning'>Delete</a></td>
					</tr>
					<?php
					$x++;
				}
